...
# Discord
DISCORD_WEBHOOK_URL=""
EVENT_SUBMISSION_DISCORD_WEBHOOK_URL=""

# Supabase (for RSVP)
SUPABASE_URL=""
SUPABASE_ANON_KEY=""
# Optional, preferred for server-side inserts:
...
`POST /api/rsvp` stores RSVP records in Supabase REST (`/rest/v1/rsvps`).

Expected table columns in Supabase:
...
If you use `SUPABASE_ANON_KEY`, configure RLS INSERT policy for the `rsvps` table. If you use `SUPABASE_SERVICE_ROLE_KEY`, inserts work server-side without exposing admin credentials to the client.

## Event Submission Webhook

`POST /api/events/submit` sends a markdown-formatted event proposal message to Discord. It uses:

1. `EVENT_SUBMISSION_DISCORD_WEBHOOK_URL` (preferred), or
2. `DISCORD_WEBHOOK_URL` (fallback)
